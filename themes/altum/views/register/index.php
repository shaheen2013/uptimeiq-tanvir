...
<form action="" method="post" class="mt-4" role="form">
    <div class="form-group">
        ...
        <input id="name" type="text" name="name" class="form-control <?= \Altum\Alerts::has_field_errors('name') ? 'is-invalid' : null ?>" value="<?= $data->values['name']=="Invited User"? '' : $data->values['name'] ?>" maxlength="32" required="required" autofocus="autofocus" <?php
        if ($data->values['name'] == "Invited User" || $data->values['name'] == "") {
            echo "";
        } else{
            echo 'readonly';
        }
    ...
    <div class="form-group">
        ...
        <input id="email" type="email" name="email" class="form-control <?= \Altum\Alerts::has_field_errors('email') ? 'is-invalid' : null ?>" value="<?= $data->values['email'] ?>" maxlength="128" required="required" <?= isset($data->values['invited']) ? 'readonly' : '' ?> />
        <?= \Altum\Alerts::output_field_error('email') ?>
    </div>


    <?php if ($data->values['type']==2 && !isset($data->values['token'] )) { ?>
        <div class="form-group">
            <label for="organization">Organization Name</label>
            <input id="organization" type="text" name="organization" class="form-control <?= \Altum\Alerts::has_field_errors('organization') ? 'is-invalid' : null ?>" value="<?= $data->values['organization'] ?>" maxlength="32"  autofocus="autofocus" />
            <?= \Altum\Alerts::output_field_error('organization') ?>
        </div>
    <?php } ?>
