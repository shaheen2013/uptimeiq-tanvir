...
<?php
if(isset($_COOKIE['heartbeat_view'])) {
    if($_COOKIE['heartbeat_view'] == "list") {
        $view_image = "fa-th";
    } else {
        $view_image = "fa-list";
    }
...
<div class="container">
    ...
    <div class="row mb-4">
        <div class="col-12 col-xl d-flex align-items-center mb-3 mb-xl-0">
            <h1 class="h4 m-0"><?= l('heartbeats.header') ?></h1>
            ...
            <div>
                <?php if ($this->user->plan_settings->heartbeats_limit != -1 && $data->total_heartbeats >= $this->user->plan_settings->heartbeats_limit) : ?>
                    <button type="button" class="btn btn-outline-primary disabled" data-toggle="tooltip" title="<?= l('global.info_message.plan_feature_limit') ?>">
                        <i class="fa fa-fw fa-sm fa-plus"></i> <?= l('heartbeats.create') ?>
                    </button>
                <?php else : ?>
                    <a href="<?= url('heartbeat-create') ?>" class="btn btn-outline-primary"><i class="fa fa-fw fa-sm fa-plus"></i> <?= l('heartbeats.create') ?></a>
                <?php endif ?>
            </div>
                    ...
                    <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                        <div class="dropdown-header d-flex justify-content-between">
                            ...
                            <?php if (count($data->filters->get)) : ?>
                                <a href="<?= url('heartbeats') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                            <?php endif ?>
                        </div>
                        ...
                        <form action="" method="get" role="form">
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                    <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('heartbeats.filters.search_by_name') ?></option>
                                </select>
                            </div>
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                    ...
                                    <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('heartbeats.filters.order_by_name') ?></option>
    ...
    <?php if (count($data->heartbeats)) : ?>
        <div class="row" id="heartbeat-blocks">
        ...
        <div class="table-responsive table-custom-container" id="heartbeat-list">
            <table class="table table-custom">
                <thead>
                <tr>
                    <th><?= l('heartbeat.table.name') ?></th>
                    <th><?= l('heartbeat.table.status') ?></th>
		    <th><?= l('heartbeat.table.uptime') ?></th>
		    <th><?= l('heartbeat.table.downtime') ?></th>
		    <th><?= l('heartbeat.table.last_run') ?></th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
                ...
                <?php foreach($data->heartbeats as $row): ?>
                    ...
                    <tr>
                        <td class="text-nowrap">
                            <a href="<?= url('heartbeat/' . $row->heartbeat_id) ?>"><?= $row->name ?></a>
                        </td>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex align-items-center">
                                <span class="font-weight-bold" data-toggle="tooltip" title="<?= sprintf(l('heartbeat.total_runs_tooltip'), nr($row->total_runs)) ?>">
                                    <?= nr($row->uptime, 3) . '%' ?>
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex align-items-center">
                                <span class="font-weight-bold" data-toggle="tooltip" title="<?= sprintf(l('heartbeat.total_missed_runs_tooltip'), nr($row->total_missed_runs)) ?>">
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex flex-column align-items-center justify-content-center py-3">
                    ...
                    <h2 class="h4 text-muted"><?= l('heartbeats.no_data') ?></h2>
                    <p class="text-muted"><?= l('heartbeats.no_data_help') ?></p>
                </div>
            </div>
        </div>
    <?php endif ?>
