...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    ...
    <div class="d-flex position-relative">
        <div class="">
            <a href="<?= url('sub-admin/user-create') ?>" class="btn btn-primary"><i class="fa fa-fw fa-plus-circle"></i> <?= l('admin_user_create.menu') ?></a>
        </div>
        ...
        <table class="table table-custom">
            ...
            <tbody>
                <?php foreach ($data->users as $row) : ?>
                    <?php
                    if (\Altum\Authentication::is_sub_admin() && \Altum\Teams::is_same_team_member(\Altum\Teams::get_main_user()->user_id,$row->user_id)) { ?>
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex flex-column">
                                <a href="<?= url('sub-admin/plan-update/' . $row->plan_id) ?>"><?= $data->plans[$row->plan_id]->name ?></a>
                        ...
                        <td>
                            <select class="user_role custom-select custom-select-sm" name="role_type" user_id="<?= $row->user_id ?>">
                                <option value="1" <?= $row->type == 1 ? 'selected' : '' ?> disabled>Site admin</option>
                                <option value="2" <?= $row->type == 2 ? 'selected' : '' ?>>Admin</option>
                                <option value="3" <?= $row->type == 3 ? 'selected' : '' ?>>Operator</option>
                                <option value="0" <?= $row->type == 0 ? 'selected' : '' ?>>Auditor</option>
                            </select>
                        </td>
                        <td>
                            <?php if (!\Altum\Teams::is_admin_check($row->user_id)) {?>
                                <div class="d-flex justify-content-end">
                                <?= include_view(THEME_PATH . 'views/sub-admin/users/admin_user_dropdown_button.php', ['id' => $row->user_id, 'resource_name' => $row->name]) ?>
                            </div>
                            <?php } ?>
                        </td>
                    </tr>
...
<!--Role change Modal -->
<div class="modal fade" user_id="" id="roleChangeConfirm" tabindex="-1" role="dialog" aria-labelledby="roleChangeConfirmLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-danger" id="roleChangeConfirmLabel">Role Change !</h5>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" target_url="<?php echo url('sub-admin/users/roleChange') ?>" id="roleChangeModalConfirm">Confirm</button>
            </div>
        </div>
    </div>
</div>
