...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    ...
    <div class="d-flex position-relative">
        <div class="">
            <a href="<?= url('sub-admin/team-create') ?>" class="btn btn-primary">Create Team</a>
            <a href="<?= url('sub-admin/team-invitation') ?>" class="btn btn-info">Invitation</a>
        </div>
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('sub-admin/teams?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item">
                        <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('sub-admin/teams?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if(count($data->filters->get)): ?>
                            <a href="<?= url('sub-admin/teams') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
...
<form id="table" action="<?= SITE_URL . 'sub-admin/teams/bulk' ?>" method="post" role="form">
        ...
        <table class="table table-custom">
            ...
            <?php foreach($data->teams as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-nowrap">
                        <div class="d-flex">
                            <a href="<?= url('sub-admin/user-view/' . $row->user_id) ?>">
                                <img src="<?= get_gravatar($row->user_email) ?>" class="user-avatar rounded-circle mr-3" alt="" />
                            </a>

                            <div class="d-flex flex-column">
                                <div>
                                    <a href="<?= url('sub-admin/user-view/' . $row->user_id) ?>"><?= $row->user_name ?></a>
                                </div>
                    ...
                    <td>
                        ...
                        <?php
                        if (\Altum\Teams::team_creator(\Altum\Teams::get_main_user()->user_id,$row->team_id)) {
                            echo include_view(THEME_PATH . 'views/sub-admin/teams/admin_team_dropdown_button.php', ['id' => $row->team_id, 'resource_name' => $row->name]);
                            }
                            ?>
                        </div>
                    </td>
...
<?php require THEME_PATH . 'views/sub-admin/partials/js_bulk.php' ?>
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/sub-admin/partials/bulk_delete_modal.php'), 'modals'); ?>
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/partials/universal_delete_modal_url.php', [
    'name' => 'team',
    'resource_id' => 'team_id',
    'has_dynamic_resource_name' => true,
    'path' => 'sub-admin/teams/delete/'
]), 'modals'); ?>
