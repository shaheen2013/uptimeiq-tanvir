...
<div class="container mt-4">
    ...
    <nav aria-label="breadcrumb">
        <ol class="custom-breadcrumbs small">
            <li>
                <a href="<?= $data->status_page->full_url ?>"><?= l('s_status_page.breadcrumb') ?></a> <div class="svg-sm text-muted d-inline-block"><?= include_view(ASSETS_PATH . '/images/s/chevron-right.svg') ?></div>
            </li>
    ...
    <div class="my-4 d-flex justify-content-between align-items-center">
        <div>
            <?php if($data->total_monitor_logs): ?>
                <span class="text-muted"><?= sprintf(l('s_monitor.total_monitor_logs'), nr($data->total_monitor_logs)) ?></span>
            <?php endif ?>
        </div>
        <button
                id="daterangepicker"
        ...
        <?php if(count($data->monitor_incidents)): ?>
            <div class="table-responsive table-custom-container">
                <table class="table table-custom">
                    <thead>
                    <tr>
                        <th colspan="3"><?= l('monitor.incidents.header') ?></th>
                    </tr>
                    <tr>
                    ...
                    <?php foreach($data->monitor_incidents as $monitor_incident): ?>
                        <tr>
                            ...
                            <td class="text-truncate text-muted">
                                <?= \Altum\Date::get($monitor_incident->end_datetime, 1) ?>
                            </td>
    ...
    <small class="text-muted"><?= sprintf(l('s_status_page.timezone'), $data->status_page->timezone) ?></small>
</div>
