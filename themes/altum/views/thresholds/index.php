...
<?php
if(isset($_COOKIE['threshold_view'])) {
    if($_COOKIE['threshold_view'] == "list") {
        $view_image = "fa-th";
    } else {
        $view_image = "fa-list";
    }
...
<div class="container">
    ...
    <div class="row mb-4">
        <div class="col-12 col-xl d-flex align-items-center mb-3 mb-xl-0">
            <h1 class="h4 m-0"><?= l('thresholds.header') ?></h1>

            <div class="ml-2">
                <span data-toggle="tooltip" title="<?= l('thresholds.subheader') ?>">
        ...
        <div class="col-12 col-xl-auto d-flex">
            <div>
                <?php if($this->user->plan_settings->thresholds_limit != -1 && $data->total_thresholds >= $this->user->plan_settings->thresholds_limit): ?>
                    <button type="button" class="btn btn-outline-primary disabled" data-toggle="tooltip" title="<?= l('global.info_message.plan_feature_limit') ?>">
                        <i class="fa fa-fw fa-sm fa-plus"></i> <?= l('thresholds.create') ?>
                    </button>
                <?php else: ?>
                    <a href="<?= url('threshold-create') ?>" class="btn btn-outline-primary"><i class="fa fa-fw fa-sm fa-plus"></i> <?= l('thresholds.create') ?></a>
                <?php endif ?>
            </div>
                    ...
                    <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                        <div class="dropdown-header d-flex justify-content-between">
                            ...
                            <?php if(count($data->filters->get)): ?>
                                <a href="<?= url('thresholds') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                            <?php endif ?>
                        </div>
                        ...
                        <form action="" method="get" role="form">
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                    <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('thresholds.filters.search_by_name') ?></option>
                                </select>
                            </div>
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                    ...
                                    <option value="last_run_datetime" <?= $data->filters->order_by == 'last_run_datetime' ? 'selected="selected"' : null ?>><?= l('thresholds.filters.order_by_last_run_datetime') ?></option>
                                    <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('thresholds.filters.order_by_name') ?></option>
                                    <option value="uptime" <?= $data->filters->order_by == 'uptime' ? 'selected="selected"' : null ?>><?= l('thresholds.filters.order_by_uptime') ?></option>
                                </select>
                            </div>
    ...
    <?php if(count($data->thresholds)): ?>
        <div class="row" id="threshold-blocks">
        ...
        <div class="table-responsive table-custom-container" id="threshold-list">
            <table class="table table-custom">
                <thead>
                <tr>
                    <th><?= l('threshold.table.name') ?></th>
                    <th><?= l('threshold.table.status') ?></th>
		    <th><?= l('threshold.table.uptime') ?></th>
		    <th><?= l('threshold.table.downtime') ?></th>
		    <th><?= l('threshold.table.last_run') ?></th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
                ...
                <?php foreach($data->thresholds as $row): ?>
                    ...
                    <tr>
                        <td class="text-nowrap">
                            <a href="<?= url('threshold/' . $row->threshold_id) ?>"><?= $row->name ?></a>
                        </td>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex align-items-center">
                                <span class="font-weight-bold" data-toggle="tooltip" title="<?= sprintf(l('threshold.total_runs_tooltip'), nr($row->total_runs)) ?>">
                                    <?= nr($row->uptime, 3) . '%' ?>
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex align-items-center">
                                <span class="font-weight-bold" data-toggle="tooltip" title="<?= sprintf(l('threshold.total_missed_runs_tooltip'), nr($row->total_missed_runs)) ?>">
                        ...
                        <td>
                            <div class="d-flex justify-content-end">
                                <?= include_view(THEME_PATH . 'views/threshold/threshold_dropdown_button.php', ['id' => $row->threshold_id, 'resource_name' => $row->name]) ?>
                            </div>
                        </td>
                    </tr>
                <?php endforeach ?>
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex flex-column align-items-center justify-content-center py-3">
                    <img src="<?= ASSETS_FULL_URL . 'images/no_rows.svg' ?>" class="col-10 col-md-7 col-lg-4 mb-3" alt="<?= l('thresholds.no_data') ?>" />
                    <h2 class="h4 text-muted"><?= l('thresholds.no_data') ?></h2>
                    <p class="text-muted"><?= l('thresholds.no_data_help') ?></p>
                </div>
            </div>
        </div>
    <?php endif ?>
