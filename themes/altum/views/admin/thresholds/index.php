...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fa fa-fw fa-xs fa-tachometer-alt text-primary-900 mr-2"></i> <?= l('admin_thresholds.header') ?></h1>
        ...
        <div class="">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/thresholds?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item">
                        <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/thresholds?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if(count($data->filters->get)): ?>
                            <a href="<?= url('admin/thresholds') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_thresholds.table.name') ?></option>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                ...
                                <option value="last_run_datetime" <?= $data->filters->order_by == 'last_run_datetime' ? 'selected="selected"' : null ?>><?= l('admin_thresholds.filters.order_by_last_run_datetime') ?></option>
                                <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_thresholds.table.name') ?></option>
                                <option value="uptime" <?= $data->filters->order_by == 'uptime' ? 'selected="selected"' : null ?>><?= l('admin_thresholds.filters.order_by_uptime') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/thresholds/bulk' ?>" method="post" role="form">
    ...
    <table class="table table-custom">
        ...
        <tr>
            ...
            <th><?= l('admin_thresholds.table.user') ?></th>
            <th><?= l('admin_thresholds.table.name') ?></th>
            <th><?= l('admin_thresholds.table.stats') ?></th>
            <th><?= l('admin_thresholds.table.is_enabled') ?></th>
            <th></th>
            <th></th>
        </tr>
        </thead>
        ...
        <?php foreach($data->thresholds as $row): ?>
            ...
            <tr>
                <td data-bulk-table class="d-none">
                    <div class="custom-control custom-checkbox">
                        <input id="selected_threshold_id_<?= $row->threshold_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->threshold_id ?>" />
                        <label class="custom-control-label" for="selected_threshold_id_<?= $row->threshold_id ?>"></label>
                    </div>
                </td>
                ...
                <td class="text-nowrap">
                    <span class="text-muted"><?= sprintf(l('admin_thresholds.table.uptime'), nr($row->uptime, 3) . '%') ?></span>
                </td>
                <td class="text-nowrap">
                ...
                <td>
                    <div class="d-flex justify-content-end">
                        <?= include_view(THEME_PATH . 'views/admin/thresholds/admin_threshold_dropdown_button.php', ['id' => $row->threshold_id, 'resource_name' => $row->name]) ?>
                    </div>
                </td>
            </tr>
        <?php endforeach ?>
