...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fa fa-fw fa-xs fa-heartbeat text-primary-900 mr-2"></i> <?= l('admin_heartbeats.header') ?></h1>
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if(count($data->filters->get)): ?>
                            <a href="<?= url('admin/heartbeats') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                ...
                                <option value="last_run_datetime" <?= $data->filters->order_by == 'last_run_datetime' ? 'selected="selected"' : null ?>><?= l('admin_heartbeats.filters.order_by_last_run_datetime') ?></option>
                                <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_heartbeats.table.name') ?></option>
                                <option value="uptime" <?= $data->filters->order_by == 'uptime' ? 'selected="selected"' : null ?>><?= l('admin_heartbeats.filters.order_by_uptime') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/heartbeats/bulk' ?>" method="post" role="form">
    ...
    <table class="table table-custom">
        ...
        <tr>
            ...
            <th><?= l('admin_heartbeats.table.user') ?></th>
            <th><?= l('admin_heartbeats.table.name') ?></th>
            <th><?= l('admin_heartbeats.table.stats') ?></th>
            <th><?= l('admin_heartbeats.table.is_enabled') ?></th>
            <th></th>
            <th></th>
        </tr>
        </thead>
        ...
        <?php foreach($data->heartbeats as $row): ?>
            ...
            <tr>
                ...
                <td class="text-nowrap">
                    <span class="text-muted"><?= sprintf(l('admin_heartbeats.table.uptime'), nr($row->uptime, 3) . '%') ?></span>
                </td>
                <td class="text-nowrap">
