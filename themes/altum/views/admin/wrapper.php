...
<body class="<?= l('direction') == 'rtl' ? 'rtl' : null ?>" data-theme-style="<?= \Altum\ThemeStyle::get() ?>">
    ...
    <script>
        $(document).on('change', '.user_create_role', function() {
            var role = $('.user_create_role').val();
            if (role == 1) {
                $('.user_create_team').hide();
                $(".user_create_team").prop('disabled', true);
            } else {
                $('.user_create_team').show();
                $(".user_create_team").prop('disabled', false);
            }
        });
    </script>
</body>
