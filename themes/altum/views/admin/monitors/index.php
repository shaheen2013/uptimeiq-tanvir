...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fa fa-fw fa-xs fa-server text-primary-900 mr-2"></i> <?= l('admin_monitors.header') ?></h1>
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if(count($data->filters->get)): ?>
                            <a href="<?= url('admin/monitors') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_monitors.table.name') ?></option>
                                <option value="target" <?= $data->filters->search_by == 'target' ? 'selected="selected"' : null ?>><?= l('admin_monitors.filters.search_by_target') ?></option>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            <label for="type" class="small"><?= l('admin_monitors.filters.type') ?></label>
                            <select name="type" id="type" class="form-control form-control-sm">
                                <option value=""><?= l('global.filters.all') ?></option>
                                <option value="website" <?= isset($data->filters->filters['type']) && $data->filters->filters['type'] == 'website' ? 'selected="selected"' : null ?>><?= l('admin_monitors.filters.type_website') ?></option>
                                <option value="ping" <?= isset($data->filters->filters['type']) && $data->filters->filters['type'] == 'ping' ? 'selected="selected"' : null ?>><?= l('admin_monitors.filters.type_ping') ?></option>
                                <option value="port" <?= isset($data->filters->filters['type']) && $data->filters->filters['type'] == 'port' ? 'selected="selected"' : null ?>><?= l('admin_monitors.filters.type_port') ?></option>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                ...
                                <option value="last_check_datetime" <?= $data->filters->order_by == 'last_check_datetime' ? 'selected="selected"' : null ?>><?= l('admin_monitors.filters.order_by_last_check_datetime') ?></option>
                                <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_monitors.table.name') ?></option>
                                <option value="uptime" <?= $data->filters->order_by == 'uptime' ? 'selected="selected"' : null ?>><?= l('admin_monitors.filters.order_by_uptime') ?></option>
                                <option value="average_response_time" <?= $data->filters->order_by == 'average_response_time' ? 'selected="selected"' : null ?>><?= l('admin_monitors.filters.order_by_average_response_time') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/monitors/bulk' ?>" method="post" role="form">
    ...
    <table class="table table-custom">
        ...
        <tr>
            ...
            <th><?= l('admin_monitors.table.user') ?></th>
            <th><?= l('admin_monitors.table.monitor') ?></th>
            <th><?= l('admin_monitors.table.stats') ?></th>
            <th><?= l('admin_monitors.table.is_enabled') ?></th>
            <th></th>
            <th></th>
        </tr>
        </thead>
        ...
        <?php foreach($data->monitors as $row): ?>
            ...
            <tr>
                ...
                <td class="text-nowrap">
                    <div class="d-flex flex-column">
                        <span class="text-muted"><?= sprintf(l('admin_monitors.table.uptime'), nr($row->uptime, 3) . '%') ?></span>
