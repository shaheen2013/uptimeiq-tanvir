...
<div class="container">
    ...
    <div class="row mb-4">
        <div class="col-12 col-xl d-flex align-items-center mb-3 mb-xl-0">
            <h1 class="h4 m-0"><?= l('domain_names.header') ?></h1>
            ...
            <div>
                <?php if($this->user->plan_settings->domain_names_limit != -1 && $data->total_domain_names >= $this->user->plan_settings->domain_names_limit): ?>
                    <button type="button" class="btn btn-outline-primary disabled" data-toggle="tooltip" title="<?= l('global.info_message.plan_feature_limit') ?>">
                        <i class="fa fa-fw fa-sm fa-plus"></i> <?= l('domain_names.create') ?>
                    </button>
                <?php else: ?>
                    <a href="<?= url('domain-name-create') ?>" class="btn btn-outline-primary"><i class="fa fa-fw fa-sm fa-plus"></i> <?= l('domain_names.create') ?></a>
                <?php endif ?>
            </div>
            ...
            <div class="ml-3">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right d-print-none">
                        <a href="<?= url('domain-names?' . $data->filters->get_get() . '&export=csv')  ?>" target="_blank" class="dropdown-item">
                            <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                        </a>
                        <a href="<?= url('domain-names?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                    ...
                    <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                        <div class="dropdown-header d-flex justify-content-between">
                            ...
                            <?php if(count($data->filters->get)): ?>
                                <a href="<?= url('domain-names') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                            <?php endif ?>
                        </div>
                        ...
                        <form action="" method="get" role="form">
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                    <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('domain_names.filters.search_by_name') ?></option>
                                    <option value="target" <?= $data->filters->search_by == 'target' ? 'selected="selected"' : null ?>><?= l('domain_names.filters.search_by_target') ?></option>
                                </select>
                            </div>
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                    ...
                                    <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('domain_names.filters.order_by_name') ?></option>
                                    <option value="target" <?= $data->filters->order_by == 'target' ? 'selected="selected"' : null ?>><?= l('domain_names.filters.order_by_target') ?></option>
                                </select>
                            </div>
    ...
    <?php if(count($data->domain_names)): ?>
        <div class="row">
            <?php foreach($data->domain_names as $row): ?>
                <?= (new \Altum\View('domain-names/domain_name_widget', (array) $this))->run(['domain_name' => $row, 'projects' => $data->projects]) ?>
            <?php endforeach ?>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex flex-column align-items-center justify-content-center py-3">
                    <img src="<?= ASSETS_FULL_URL . 'images/no_rows.svg' ?>" class="col-10 col-md-7 col-lg-4 mb-3" alt="<?= l('domain_names.no_data') ?>" />
                    <h2 class="h4 text-muted"><?= l('domain_names.no_data') ?></h2>
                    <p class="text-muted"><?= l('domain_names.no_data_help') ?></p>
                </div>
            </div>
        </div>
