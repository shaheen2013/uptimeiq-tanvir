...
class Heartbeats extends Controller {
    ...
    public function index() {
        ...
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `heartbeats` WHERE `team_id` = {$main_user->team_id} {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('heartbeats?' . $filters->get_get() . '&page=%d')));

        /* get tags */
        $all_tags=db()->where('user_id', $main_user->user_id)->where('heartbeat_id', 'null',"!=")->get('tags', null, ['name']);
        $tags=[];
        ...
        process_export_json($heartbeats, 'include', ['heartbeat_id', 'project_id', 'name', 'code', 'is_ok', 'uptime', 'downtime', 'total_runs', 'total_missed_runs', 'last_run_datetime', 'next_run_datetime', 'notifications', 'is_enabled', 'datetime'], sprintf(l('heartbeats.title')));
