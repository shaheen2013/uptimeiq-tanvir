    ...
    public function index()
    {
        ...
        if (!empty($_POST)) {
            ...
            if (!Alerts::has_field_errors() && !Alerts::has_errors() && !Alerts::has_infos()) {
                ...
                // log into default team
                $logged_in_user = db()->where('user_id', $user->user_id)->getOne('users', 'type');

                $total_teams = db()->where('user_id', $user->user_id)->where('status', 1)->where('default_member', 1)->get('teams_members', null, 'team_member_id');
