...
class ThresholdCreate extends Controller {
    ...
    public function index() { 
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('create.thresholds')) {
        ...
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `thresholds` WHERE `user_id` = {$main_user->user_id}")->fetch_object()->total ?? 0;

        if($main_user->plan_settings->thresholds_limit != -1 && $total_rows >= $main_user->plan_settings->thresholds_limit) {
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                $settings = json_encode([
                    ...
                    'threshold_breach_value_type' => $_POST['threshold_breach_value_type'] ?? 'none',
                ...
                /* create incident for current threshold */
                ...
                /* update threshold id for incident */
                $incident_id = db()->update('incidents',[
                    'threshold_id'=>$threshold_id
                ]);
                
                /* Set a nice success message */
                Alerts::add_success(l('threshold_create.success_message'));

                redirect('threshold/' . $threshold_id);
            }
        ...
        $view = new \Altum\View('threshold-create/index', (array) $this);
