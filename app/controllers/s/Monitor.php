...
class Monitor extends Controller {
    ...
    public function index() {
        ...
        Meta::set_social_image(!empty($this->status_page->opengraph) ? UPLOADS_FULL_URL . 'status_pages_opengraph/' . $this->status_page->opengraph : null);
    ...
    public function init() {
        ...
        if(isset(\Altum\Router::$data['domain']) && \Altum\Router::$data['domain']->status_page_id) {
            $monitor_id = isset($this->params[0]) ? query_clean($this->params[0]) : null;
        } else {
            $monitor_id = isset($this->params[1]) ? query_clean($this->params[1]) : null;
        }
