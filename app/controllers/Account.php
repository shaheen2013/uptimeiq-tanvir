...
class Account extends Controller {
    ...
    public function index() {
        ...
        $twofa_image = $twofa->getQRCodeImageAsDataUri($main_user->email . ' - ' . $main_user->name, $twofa_secret);
        ...
        if(!empty($_POST)) {
            ...
            $twofa_secret               = $_POST['twofa_is_enabled'] ? $main_user->twofa_secret : null;

            /* Billing */
            if(empty($main_user->payment_subscription_id)) {
            ...
            if(db()->where('email', $_POST['email'])->has('users') && $_POST['email'] !== $main_user->email) {
            ...
            if(!empty($_POST['old_password']) && !empty($_POST['new_password'])) {
                if(!password_verify($_POST['old_password'], $main_user->password)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                if(!empty($main_user->payment_subscription_id)) {
                    $_POST['billing'] = json_encode($main_user->billing);
                }

                /* Database query */
                db()->where('user_id', $main_user->user_id)->update('users', [
                    'name' => $_POST['name'],
                ...
                if($_POST['email'] != $main_user->email) {
                    ...
                    if(settings()->users->email_confirmation) {
                        ...
                        $email_template = get_email_template(
                            ...
                            [
                                ...
                                '{{NAME}}' => $main_user->name,
                                '{{CURRENT_EMAIL}}' => $main_user->email,
                                '{{NEW_EMAIL}}' => $_POST['email'],
                            ],
                            l('global.emails.user_pending_email.body')
                        );

                        send_mail($_POST['email'], $email_template->subject, $email_template->body, ['anti_phishing_code' => $main_user->anti_phishing_code, 'language' => $main_user->language]);
                        ...
                        db()->where('user_id', $main_user->user_id)->update('users', ['email' => $_POST['email']]);
                ...
                if(!empty($_POST['old_password']) && !empty($_POST['new_password'])) {
                    ...
                    db()->where('user_id', $main_user->user_id)->update('users', ['password' => $new_password]);
                    ...
                    /* Clear the cache */
                    \Altum\Cache::$adapter->deleteItemsByTag('user_id=' . $main_user->user_id);
