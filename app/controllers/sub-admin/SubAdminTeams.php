...
class SubAdminTeams extends Controller
{
    ...
    public function index()
    {
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('sub-admin/teams?' . $filters->get_get() . '&page=%d')));

        /* Get the data */
        $teams = [];

        $accessible_team = [];
        $team_create = db()->where('user_id', \Altum\Teams::get_main_user()->user_id)->get('teams');
        foreach ($team_create as $key => $value) {
            array_push($accessible_team, $value->team_id);
        }

        $joined_team = db()->where('user_id', \Altum\Teams::get_main_user()->user_id)->where('status',1)->get('teams_members');
        foreach ($joined_team as $key => $value) {
            array_push($accessible_team, $value->team_id);
        }

        $accessible_team_string = implode(',', array_unique($accessible_team));
        ...
        $teams_result = database()->query("
            ...
            WHERE
                1 = 1
                And `teams`.`team_id` IN ($accessible_team_string)
        ...
        $view = new \Altum\View('sub-admin/teams/index', (array) $this);
    ...
    public function delete()
    {
        ...
        if ($team = db()->where('team_id', $team_id)->where('default_member', 1)->getOne('teams_members', ['team_id'])) {
            Alerts::add_error('Delete default team user first!');
        }
