...
class AdminTeamInvitation extends Controller
{
    public function index() {
        ...
        $team_total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `teams_members` WHERE (`user_id` = {$this->user->user_id} OR `user_email` = '{$this->user->email}') {$team_filters->get_sql_where()}")->fetch_object()->total ?? 0;
        ...
        $teams_member_result = database()->query("
            ...
            WHERE 
                  (`teams_members`.`user_id` = {$this->user->user_id} AND `teams_members`.`status` = 0 
                  OR `teams_members`.`user_email` = '{$this->user->email}' AND `teams_members`.`status` = 0 )
        ...
        $view = new \Altum\View('admin/team-invitation/index', (array) $this);
