...
class AdminUserCreate extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($teams_id as $k => $val) {
            $team_details = db()->where('team_id', $val)->getOne('teams');
            $team[$k] = $team_details;
        }
        ...
        if (!empty($_POST)) {
            ...
            if (!in_array($_POST['role'], [0, 1, 2, 3])) {
                Alerts::add_field_error('role', 'Choose given option!');
            }
            if ($_POST['role'] == 1 && isset($_POST['team'])) {
                Alerts::add_field_error('team', 'You cannot choose team for Site-admin!');
            }
            if (!isset($_POST['team']) && !isset($_POST['organization_name']) && $_POST['role'] != 1) {
                Alerts::add_field_error('role', 'Choose team or organization for this role!');
            }
            ...
            if (!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                if (settings()->webhooks->user_new) {
                    ...
                    \Unirest\Request::post(settings()->webhooks->user_new, [], [
                        ...
                        'name' => $_POST['name'],
                        'type' => $_POST['role'],
                        'source' => 'site_admin_create',
                    ]);
                }

                if ($_POST['role'] != 1) {
                    if (isset($_POST['organization_name'])) {
                        /* Prepare the statement to create team for logged in user*/
                        ...
                        // team member create for the logged in user
