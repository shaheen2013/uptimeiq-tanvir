...
class Projects extends Controller {
    ...
    public function index() {
        ...
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `projects` WHERE `team_id` = {$main_user->team_id} {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
        ...
        $projects = [];
        $projects_result = database()->query("SELECT * FROM `projects` WHERE `team_id` = {$main_user->team_id} {$filters->get_sql_where()} {$filters->get_sql_order_by()} {$paginator->get_sql_limit()}");
    ...
    public function delete() {
        ...
        if(!$project = db()->where('project_id', $project_id)->where('team_id', $main_user->team_id)->getOne('projects', ['project_id', 'name'])) {
            redirect('projects');
        }
